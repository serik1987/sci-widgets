...
Сам живой поиск определён функцией updateFunc(). Задача этой функции - вернуть промис, запускающий отправку данных
...
Функция updateFunc должна в качестве аргумента принимать введённую пользователем строку, она всегда будет выполняться
...
Задача этой функции - вернуть промис, запускающий отправку данных
по HTTP. В случае успешного получения результата промис должен разрешаться успешно. Помните, что когда промис
находится в состоянии ожидания, то функция readyFunc() должна возвращать false, а когда промис разрешен - то true.

Функция updateFunc должна в качестве аргумента принимать введённую пользователем строку и всегда выполняется
в контексте вызывающего её элемента sci-input.

Если Вы проставите атрибут livesearch, но при этом не назначите в качестве значения атрибута updateFunc некоторую
функцию, то будет сгенерировано исключение типа TypeError. Данная функция не запустится, если не проставлен
атрибут livesearch, либо если функция readyFunc возвращает false (например, у Вас значение свойства ready равно
false).
